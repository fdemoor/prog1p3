...
\paragraph{}First, we will introduce the game and its features. Then, we will expose the class diagram, and the structure we decided to use to create this game. Finally, we will clarify some parts of the implementation.
...
To structure the code, we decided to make packages. We can classify them into two types :
\begin{itemize}
	\item The packages \emph{insect}, \emph{colony}, \emph{places}, \emph{projectiles} are managing the game engine.
	\item The package \emph{mvc} is managing all the graphic interface.
\end{itemize}
Here is below the class diagram of our implementation :

\begin{figure}[H]
	\center
	\includegraphics[scale=0.7]{classDiagram.png} % Legend ??
